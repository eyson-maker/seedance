...
/**
 * Generations table for tracking Seedance 2.0 video generations
 */
export const generation = pgTable("generation", {
	...
	duration: integer("duration").default(5), // video duration in seconds
	resolution: text("resolution").default('720p'),
	metadata: text("metadata"), // JSON string for ref types, error info, etc.
	createdAt: timestamp("created_at").notNull().defaultNow(),
	updatedAt: timestamp("updated_at").notNull().defaultNow(),
